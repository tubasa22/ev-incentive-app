...
충전기 설치·전기패널 업그레이드 공사비가 지원금 한도를 초과하면 차액은 고객이 부담합니다. 관리자 화면의 **케이스 검토**에서 케이스를 검색한 뒤 예상공사비와 예상지원금한도를 입력하면 고객부담예상액이 자동 계산되어 Cases에 저장됩니다. 미입력과 0달러는 구분되며, 지원금이 공사비 이상이면 고객부담액은 0입니다. 관리자는 고객에게 전화/이메일로 견적과 차액을 안내하고 공사 진행 의사를 별도로 확인합니다. 자동 이메일과 결제 연동은 후속 작업입니다.

새 견적 저장 기능을 사용하려면 변경된 `apps-script/Code.gs`를 Apps Script에 반영하고 웹앱을 새 버전으로 배포해야 합니다.

신청 대행 이용료는 차량 프로그램만 진행할 때 $99, 충전기 설치가 포함될 때 $199입니다. 충전기 시공을 진행하면 이용료는 시공비에서 전액 차감됩니다. 고객은 신청서 마지막 단계에서 환불정책과 제한된 대행 범위를 끝까지 확인하고 전자서명해야 제출할 수 있으며, 동의 여부·시각·서명·적용 이용료가 Cases의 신청자정보(JSON)에 저장됩니다. 현재는 가격 표시와 동의 기록만 구현되어 있고 실제 Stripe 결제는 후속 작업입니다.
