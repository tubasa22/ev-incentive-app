...
- 이미지: `site-config.js`의 `SITE_CONFIG.images`에서 Unsplash hotlink를 관리한다. 히어로는 가정 충전 EV, 소개 페이지는 따뜻한 가족 사진을 사용하며, 실제 사진으로 바꿀 위치에는 TODO 주석을 남긴다.
...
- [x] 클린EV 공개 페이지 디자인: 고정 팔레트·Pretendard·Unsplash 임시 이미지·모바일 우선 레이아웃 적용
