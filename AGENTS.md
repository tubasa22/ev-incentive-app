...
- 무료 인테이크 제출 시 매칭 룰 엔진을 실행해 신청·매칭 정보를 Leads에 저장한다. 가능성 유무만 화면과 이메일로 안내하며, 구체적인 프로그램명·예상금액은 고객에게 노출하지 않는다.
...
- 활성 프로그램 가능성이 있으면 `가능성있음`, 없으면 `부적격`으로 기록한다. 고객 화면과 이메일에는 구체적 프로그램명·예상금액을 노출하지 않고 가능성 유무만 안내한다.
...
- 자격확인(무료)과 결제(유료)를 `Leads` 시트로 분리했다. 자격확인 후 가능성 유무만 이메일로 안내하고, 가능성 있는 고객은 고유 링크로 저장 정보를 재사용해 동의·결제를 진행한다. 이메일에는 프로그램명·예상금액을 노출하지 않으며, 결제 확인 후에만 Cases가 생성되고 리드가 전환완료된다. `admin.html`에서 상태별 리드 관리와 7일 이상 미전환 리드 안내 재발송을 지원한다.
