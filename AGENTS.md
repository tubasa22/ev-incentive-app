...
- 이번 단계에서는 가격 표시와 동의 기록만 제공하며 Stripe 등 실제 결제 연동은 별도 작업으로 남긴다.
...
- 신청 대행 동의서 도입, 가격체계(`CONFIG.pricing`) 반영 (환불 불가·시공비 차감 정책), 실제 결제 연동은 별도 작업으로 남음
...
- 견적 금액 고객 자동 이메일 안내 검토 (현재는 관리자 수동 안내), 결제 연동은 별도 작업
