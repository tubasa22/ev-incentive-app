...
- 이용료는 실제 제공된 자격 검토·서류 준비·현장 방문/견적·신청서 작성 및 제출 대행의 대가로 환불되지 않는다. 이후 충전기 시공을 진행하면 이용료를 시공비에서 전액 차감한다.
...
- 신청 대행 동의서와 가격체계(`CONFIG.pricing`) 반영 (환불 불가·시공비 차감 정책). 결제 흐름 구현 완료, 실제 Stripe 계정·링크·비밀키 설정 및 운영 검증 필요
...
- 신청 대행 동의서와 환불정책은 실제 법적 효력이 필요한 문서이므로 변호사 검토 후 확정 필요
